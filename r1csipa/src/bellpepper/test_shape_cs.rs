...
#[allow(dead_code)] // the first two fields are never read
#[derive(Debug)]
enum NamedObject {
    Constraint(usize),
    Var(Variable),
...
/// `TestShapeCS` is a `ConstraintSystem` for creating `R1CSShape`s for a
/// circuit.
pub struct TestShapeCS<F: PrimeField> {
...
impl<F: PrimeField> TestShapeCS<F> {
    #[allow(unused)]
    ...
    pub fn pretty_print(&self) -> String {
        ...
        let powers_of_two = (0..F::NUM_BITS)
            .map(|i| F::from(2u64).pow_vartime([u64::from(i)]))
            .collect::<Vec<_>>();

        let pp = |s: &mut String, lc: &LinearCombination<F>| {
            s.push('(');
            let mut is_first = true;
            for (var, coeff) in proc_lc::<F>(lc) {
                if coeff == negone {
                    s.push_str(" - ")
                } else if !is_first {
                    s.push_str(" + ")
...
impl<F: PrimeField> Default for TestShapeCS<F> {
    fn default() -> Self {
        let mut map = HashMap::new();
        map.insert("ONE".into(), NamedObject::Var(TestShapeCS::<F>::one()));
        TestShapeCS {
            named_objects: map,
...
impl<F: PrimeField> ConstraintSystem<F> for TestShapeCS<F> {
    type Root = Self;
