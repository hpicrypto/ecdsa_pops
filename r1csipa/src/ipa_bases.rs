use halo2curves::{ff::Field, CurveAffine};
use rayon::prelude::*;

use crate::msm_function;

// Represent each base symbolically, as (bases, scalars), such that base =
// msm(bases, scalars) When we later use the base in an MSM, say as base^x, we
// compute (bases, x*scalars) and add this list of terms (bases[1]^scalars[1],
// ..., ) as terms in the MSM
#[derive(Clone)]
...
impl<C: CurveAffine<ScalarExt = F>, F: Field> IPABases<C, F> {
    ...
    // Update G_L[i] to  G_L[i] * scalars[0], (just update the scalars, don't
    // actually compute the scalar mult) then append G_R[i] * scalars[1], (again
    // not the scalar mult)
    ...
    /// 1. A vector of scalars (combining the input scalars with the internal
    ///    scalars)
    ...
    // collapse: calls msm(bases[i], scalars[i]) for all i, sets bases[i] to msm
    // output, scalars[i] = 1
    #[allow(dead_code)]
