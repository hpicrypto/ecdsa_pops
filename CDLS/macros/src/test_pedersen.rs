...
macro_rules! __test_pedersen {
    ($config: ty, $OtherProjectiveType: ty) => {
        ...
        #[test]
        fn test_pedersen_sq() {
            let label = b"PedersenSq";
            let m = SF::rand(&mut OsRng);
            let m_sq = m * m;

            let c1: PC = PC::new(m, &mut OsRng);
            let c2: PC = PC::new(m_sq, &mut OsRng);

            let mut transcript = Transcript::new(label);
            let proof = SqProof::<$config>::create(&mut transcript, &mut OsRng, &m, &c1, &c2);

            // The proof should verify with the same label.
            let mut transcript_v = Transcript::new(label);
            assert!(proof.verify(&mut transcript_v, &c1.comm, &c2.comm));

            // And should reject when the second commitment isn't to m^2.
            let mut d = SF::rand(&mut OsRng);
            while d == m_sq {
                d = SF::rand(&mut OsRng);
            }
            let c_wrong: PC = PC::new(d, &mut OsRng);
            let mut transcript_w = Transcript::new(label);
            assert!(!proof.verify(&mut transcript_w, &c1.comm, &c_wrong.comm));
        }
        ...
        #[test]
        fn test_opt_ec_point_add() {
            let label = b"PedersenOptECPointAdd";
            let a = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            let mut b = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            while b == a {
                b = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            ...
            let mut transcript = Transcript::new(label);
            let proof = OptEPAP::<$config>::create(
                &mut transcript,
                &mut OsRng,
                a,
                b,
            ...
            // C_τ and all T_i / U_i should be on the curve.
            assert!(proof.c_tau.is_on_curve());
            assert!(proof.t1.is_on_curve());
            assert!(proof.t2.is_on_curve());
            assert!(proof.t3.is_on_curve());
            assert!(proof.t4.is_on_curve());
            assert!(proof.t5.is_on_curve());
            assert!(proof.t6.is_on_curve());
            assert!(proof.u1.is_on_curve());
            assert!(proof.u2.is_on_curve());
            assert!(proof.u3.is_on_curve());
            ...
            // Negative test: forge a proof for a wrong target t_f ≠ t.
            let mut tf = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            while tf == t {
                tf = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            }
            ...
            let mut transcript_f = Transcript::new(label);
            let proof_f = OptEPAP::<$config>::create(
                &mut transcript_f,
                &mut OsRng,
                a,
                b,
        ...
        #[test]
        fn test_standalone_opt_ec_point_add() {
            let label = b"PedersenSqECPointAdd";
            let a = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            let mut b = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            while b == a {
                b = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            ...
            let mut transcript = Transcript::new(label);
            let proof = SqEPAP::<$config>::create(
                &mut transcript,
                &mut OsRng,
                a,
                b,
            ...
            assert!(proof.t5.is_on_curve());
            assert!(proof.t6.is_on_curve());
            assert!(proof.t7.is_on_curve()); // T_7 retained for standalone.
            assert!(proof.u1.is_on_curve());
            assert!(proof.u2.is_on_curve());
            assert!(proof.u3.is_on_curve());

            // Completeness: verify with a matching transcript.
            let mut transcript_v = Transcript::new(label);
            assert!(proof.verify(
                &mut transcript_v,
                &cax.comm,
            ...
            // Negative test: forge a proof for a wrong target t_f ≠ a + b.
            let mut tf = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            while tf == t {
                tf = <$OtherProjectiveType>::rand(&mut OsRng).into_affine();
            }
            ...
            let mut transcript_f = Transcript::new(label);
            let proof_f = SqEPAP::<$config>::create(
                &mut transcript_f,
                &mut OsRng,
                a,
                b,
        ...
        #[test]
        fn test_wc_scalar_mult() {
            let label = b"PedersenWCScalarMult";
            let lambda = OSF::rand(&mut OsRng);
            let s = (OGENERATOR.mul(lambda)).into_affine();

            let c_zx = PC::new(<$config as PedersenConfig>::from_ob_to_sf(s.x), &mut OsRng);
            let c_zy = PC::new(<$config as PedersenConfig>::from_ob_to_sf(s.y), &mut OsRng);

            let mut transcript = Transcript::new(label);
            let proof = ECSMWCP::<$config>::create(
                &mut transcript,
                &mut OsRng,
                &s,
                &lambda,
            ...
            let mut transcript_v = Transcript::new(label);
            let result = proof.verify(&mut transcript_v, &OGENERATOR, &c_zx.comm, &c_zy.comm);
            assert!(result);
        }

        #[test]
        fn test_fs_wc_scalar_mult() {
            let label = b"PedersenFSWCScalarMult";
            let lambda = OSF::rand(&mut OsRng);
            let s = (OGENERATOR.mul(lambda)).into_affine();
            ...
            let mut transcript = Transcript::new(label);
            let proof = FSECMWCP::<$config>::create(
                &mut transcript,
                &mut OsRng,
                &s,
                &lambda,
            ...
            let mut transcript_v = Transcript::new(label);
            assert!(proof.verify(&mut transcript_v, &OGENERATOR, &c_zx.comm, &c_zy.comm));
        }
        ...
        #[test]
        fn test_ecdsa_wc_signature() {
            // ECDSA signature knowledge proof against a committed public key, using
            // the WC scalar-mul + Sq-PA standalone variant.
            let label = b"ECDSASigWC";
            type OScalar = <<$config as PedersenConfig>::OCurve as CurveConfig>::ScalarField;

            // Generate an ECDSA-style witness:
            //   - secret key sk ∈ F_q
            //   - public key Q = sk·G
            //   - message hash t ∈ F_q (arbitrary scalar)
            //   - ephemeral key k ∈ F_q (random nonce)
            //   - R = k·G
            //   - r_x = R.x  (used as the truncated x-coordinate)
            //   - s = k⁻¹ · (t + r_x · sk)
            //
            // The verifier later checks z·R = t·r_x⁻¹·G + Q where z = s · r_x⁻¹.
            let sk = OScalar::rand(&mut OsRng);
            let q = (<<$config as PedersenConfig>::OCurve as SWCurveConfig>::GENERATOR
                .into_group()
                * sk)
                .into_affine();

            let t_scalar = OScalar::rand(&mut OsRng);

            // Sample a non-zero ephemeral k.
            let k = loop {
                let cand = OScalar::rand(&mut OsRng);
                if !cand.is_zero() {
                    break cand;
                }
            };
            let r_pt = (<<$config as PedersenConfig>::OCurve as SWCurveConfig>::GENERATOR
                .into_group()
                * k)
                .into_affine();
            let r_x_ob = *r_pt.x().unwrap();
            let r_x = <$config as PedersenConfig>::from_ob_to_os(r_x_ob);

            // s = k⁻¹ · (t + r_x · sk).
            let s = k.inverse().unwrap() * (t_scalar + r_x * sk);

            // Prove.
            let mut transcript_p = Transcript::new(label);
            ...
            let mut transcript_v = Transcript::new(label);
            assert!(
                proof.verify(&mut transcript_v, &r_pt, &t_scalar),
                "honest proof failed to verify"
            );

            // Negative test: change the message hash (verifier expects t but tries t' ≠ t).
            let mut t_wrong = OScalar::rand(&mut OsRng);
            while t_wrong == t_scalar {
                t_wrong = OScalar::rand(&mut OsRng);
            }
            let mut transcript_wrong = Transcript::new(label);
            assert!(
                !proof.verify(&mut transcript_wrong, &r_pt, &t_wrong),
                "proof verified against wrong message hash"
            );
        }
    ...
    ($mod_name: ident; $config: ty, $OtherProjectiveType: ty) => {
        mod $mod_name {
            ...
            use pedersen::{
                ...
                ec_point_add_protocol_opt::{
                    OptECPointAddIntermediate as OptEPAI, OptECPointAddProof as OptEPAP,
                },
                ec_point_add_protocol_opt_alone::{
                    SqECPointAddIntermediate as SqEPAI, SqECPointAddProof as SqEPAP,
                },
                ...
                fs_scalar_mul_wc_protocol::FSECScalarMulWCProof as FSECMWCP,
                ...
                scalar_mul_wc_protocol::{
                    ECScalarMulWCProof as ECSMWCP, ECScalarMulWCProofIntermediate as ECSMWCPI,
                },
                sq_protocol::{
                    SqProof, SqProofIntermediate, SqProofIntermediateTranscript,
